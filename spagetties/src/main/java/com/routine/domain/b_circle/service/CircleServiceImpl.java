...
public class CircleServiceImpl implements CircleService {
    ...
    public void transferLeadership(Long circleId, Long currentLeaderId, Long newLeaderId) {
        ...
        CircleMember newLeaderMember = circle.getMembers().stream()
                .filter(cm -> cm.getMember().getId().equals(newLeaderId))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("해당 멤버는 써클에 소속되어 있지 않습니다."));

        CircleMember currentLeaderMember = circle.getMembers().stream()
                .filter(cm -> cm.getMember().getId().equals(currentLeaderId))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("현재 리더를 찾을 수 없습니다."));


        circle.setCreatorId(newLeaderId);
        newLeaderMember.setRole(CircleMember.Role.LEADER);


        currentLeaderMember.setRole(CircleMember.Role.MEMBER);


        circleRepository.save(circle);
        circleMemberRepository.save(newLeaderMember);
        circleMemberRepository.save(currentLeaderMember);
    }
    ...
    @Override
    public List<MyCircleJoinStatusDto> getMyJoinStatusList(Long memberId, String statusFilter) {
        List<CircleMember> myCircleMembers = circleMemberRepository.findByMemberIdOrderByCreatedAtDesc(memberId);

        return myCircleMembers.stream()
                .filter(cm -> statusFilter == null || cm.getStatus().name().equalsIgnoreCase(statusFilter))
                .map(cm -> new MyCircleJoinStatusDto(
                        cm.getCircle().getId(),
                        cm.getCircle().getName(),
                        cm.getStatus().name()
                ))
                .collect(Collectors.toList());
    }
